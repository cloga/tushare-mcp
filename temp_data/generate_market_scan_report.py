...
# List of stocks from the previous scan
stocks_data = [
    ...
    {"ts_code":"000898.SZ","name":"鞍钢股份","industry":"普钢"},
    ...
    {"ts_code":"300402.SZ","name":"宝色股份","industry":"专用机械"},
    ...
    {"ts_code":"600399.SH","name":"抚顺特钢","industry":"特种钢"},
...
# Determine the latest trade date
today = datetime.datetime.now().strftime('%Y%m%d')
cal = pro.trade_cal(exchange='', start_date='20251101', end_date=today)
last_trade_date = cal[cal.is_open==1].iloc[-1]['cal_date']
print(f"Fetching data for date: {last_trade_date}")

# Fetch Daily Basic (PE, PB, Dividend Yield)
# fields: ts_code, trade_date, close, pe, pe_ttm, pb, dv_ratio, turnover_rate
df_basic = pro.daily_basic(trade_date=last_trade_date, fields='ts_code,pe,pe_ttm,dv_ratio,turnover_rate')

# Fetch Daily Price (Close, Change)
# fields: ts_code, trade_date, close, pct_chg
df_daily = pro.daily(trade_date=last_trade_date, fields='ts_code,close,pct_chg')

# Merge data
df_merged = pd.merge(df_stocks, df_basic, on='ts_code', how='left')
df_merged = pd.merge(df_merged, df_daily, on='ts_code', how='left')
    ...
    <table id="stockTable">
        <thead>
            <tr>
                ...
                <th onclick="sortTable(3)">现价</th>
                <th onclick="sortTable(4)">涨跌幅(%)</th>
                <th onclick="sortTable(5)">PE(TTM)</th>
                <th onclick="sortTable(6)">股息率(%)</th>
            </tr>
        </thead>
        <tbody>
"""
...
for index, row in df_merged.iterrows():
    ...
    html_content += f"""
            <tr>
                ...
                <td>{row['industry']}</td>
                <td>{row['close']}</td>
                <td class="{pct_class}">{row['pct_chg']}</td>
                <td>{row['pe_ttm']}</td>
                <td>{row['dv_ratio']}</td>
            </tr>
    """
...
output_path = os.path.join('temp_data', 'market_scan_report.html')
